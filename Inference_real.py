...
def main():
    ...
    rospy.Subscriber('/rgb_to_depth/image_raw', ROSImage, rgbd_depth_image_callback)
    rospy.Subscriber('/depth/image_raw', ROSImage, depth_image_callback)
    rospy.Subscriber('/depth/camera_info', CameraInfo, info_callback)
    rate = rospy.Rate(100)
    ...
    while not rospy.is_shutdown():
        ...
        if new_input_available:
            ...
            # point_list = [[int(num) for num in current_input.split()]]
            # print(point_list)
            # point_label=[1]
            # ann = prompt_process.point_prompt(points= point_list, pointlabel=point_label)
