...
{
    public class RightLoadSubscriber : Subscriber<Messages.Sensor.Joy>
    {
        public JoyButtonWriter[] joyButtonWriters;
        public JoyAxisWriter[] joyAxisWriters;
        public string configName = "Right Device";
        private double[] hapticForce = new double[3];
        private double[] hapticTorque = new double[3] { 0, 0, 0 };
        ...
        protected override void ReceiveMessage(Messages.Sensor.Joy joy)
        {
            ...
            for (int i=0;i<3;i++)
            {
                hapticForce[i] = joy.axes[i];
            }
            //print(hapticForce);
            HapticPlugin.setForce(configName, hapticForce, hapticTorque);
        }
    }
}
