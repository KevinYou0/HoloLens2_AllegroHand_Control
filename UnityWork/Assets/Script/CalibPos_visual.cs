...
public class CalibPos_visual : MonoBehaviour
{
    ...
    Vector3 pos_cylinder = new Vector3(0.52094144f, -0.11900946f, 0.314116638f);
    Vector3 pos_star = new Vector3(0.52141990f, 0.00945116f, 0.31992781f);
    Vector3 pos_small_cylinder = new Vector3(0.52100867f, 0.12295886f, 0.31849644f);
    ...
    void Start()
    {
        lineRenderer = vis_line.GetComponent<LineRenderer>();
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            lineRenderer.SetPosition(1, hit.point); // Ends at the collision point
        }
        else
        {
            // Set the line to extend in the forward direction of the camera with a specific length
            lineRenderer.SetPosition(0, Camera.main.transform.position);
            lineRenderer.SetPosition(1, ray.origin + ray.direction * 100); // Arbitrary length
        }
        ...
        if (Physics.Raycast(headPosition, gazeDirection, out hit))
        {
            ...
            string hitObjectName = hit.collider.gameObject.tag;
            ...
            if (IsMoving && !on_mission)
            {
                ...
                if (hitObjectName == "round" || hitObjectName == "star" || hitObjectName == "cylinder") 
                {
                    ...
                    move_count += 1;
                    obj_lb = 1;
                    // change between the noise/un-noise trial
                    Vector3 localPosition = localTran.InverseTransformPoint(hit.point);
                    if (hitObjectName == "round") 
                    {
                        temp_obj = pos_small_cylinder;
                    }
                    if (hitObjectName == "star")
                    {
                        temp_obj = pos_star;
                    }
                    if (hitObjectName == "cylinder")
                    {
