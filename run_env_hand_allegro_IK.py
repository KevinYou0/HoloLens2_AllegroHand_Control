...
def calculate_plane_normal(p1, p2, p3): #0,6,21
    v1 = p2 - p1 #0->6
    v2 = p3 - p1 #0->21
    ...
    try:
        ...
        while enable and not rospy.is_shutdown():
            ...
            if (si.is_valid_hand_right()):
                ...
                points2 = np.squeeze(np.asarray(right_joints.positions[21]))
                points3 = np.squeeze(np.asarray(right_joints.positions[6]))

                return_x = calculate_plane_normal(points1, points2, points3)
                return_z = ((np.squeeze(np.asarray(right_joints.positions[0])) - np.squeeze(
                    np.asarray(right_joints.positions[1]))) /
                                       norm(np.squeeze(np.asarray(right_joints.positions[1])) - np.squeeze(
                                           np.asarray(right_joints.positions[0]))))
                return_y = -np.cross(np.squeeze(np.asarray(return_x)),
                                             np.squeeze(np.asarray(return_z)))
                thumb_relative_vec = np.asarray(right_joints.positions[5]) - np.asarray(right_joints.positions[0])
                index_relative_vec = np.asarray(right_joints.positions[10]) - np.asarray(right_joints.positions[0])
                middle_relative_vec = np.asarray(right_joints.positions[15]) - np.asarray(right_joints.positions[0])
                ring_relative_vec = np.asarray(right_joints.positions[20]) - np.asarray(right_joints.positions[0])

                target_thumb = [np.dot(thumb_relative_vec, return_x),
                                np.dot(thumb_relative_vec, return_y),
                                np.dot(thumb_relative_vec, return_z)]
                frame_target_thumb = np.eye(4)
                frame_target_thumb[:3, 3] = target_thumb

                target_index = [np.dot(index_relative_vec, return_x),
                                np.dot(index_relative_vec, return_y),
                                np.dot(index_relative_vec, return_z)]
                frame_target_index = np.eye(4)
                frame_target_index[:3, 3] = target_index

                target_middle = [np.dot(middle_relative_vec, return_x),
                                np.dot(middle_relative_vec, return_y),
                                np.dot(middle_relative_vec, return_z)]
                frame_target_middle = np.eye(4)
                frame_target_middle[:3, 3] = target_middle

                target_ring = [np.dot(ring_relative_vec, return_x),
                                np.dot(ring_relative_vec, return_y),
                                np.dot(ring_relative_vec, return_z)]
                frame_target_ring = np.eye(4)
                frame_target_ring[:3, 3] = target_ring

                ik_thumb = allegro_thumb_chain.inverse_kinematics_frame(frame_target_thumb)
                ik_index = allegro_thumb_chain.inverse_kinematics_frame(frame_target_index)
                ik_middle = allegro_thumb_chain.inverse_kinematics_frame(frame_target_middle)
                ik_ring = allegro_thumb_chain.inverse_kinematics_frame(frame_target_ring)

                thumb_angles = [ik_thumb[1], ik_thumb[2], ik_thumb[3], ik_thumb[4]]
                index_angles = [ik_index[1], ik_index[2], ik_index[3], ik_index[4]]
                middle_angles = [ik_middle[1], ik_middle[2], ik_middle[3], ik_middle[4]]
                ring_angles = [ik_ring[1], ik_ring[2], ik_ring[3], ik_ring[4]]
                ...
                hl2ss_utilities.draw_points(image, right_image_points.astype(np.int32)[[5]], radius,
                                            ch_color, thickness)

                # Assuming angle_lists is updated within the loop
                joy_msg = Joy()
                joy_msg.header.stamp = rospy.Time.now()
                joy_msg.axes = [angle_lists[1][0], angle_lists[1][1], angle_lists[1][2], angle_lists[1][3],
                                angle_lists[2][0], angle_lists[2][1], angle_lists[2][2], angle_lists[2][3],
                                angle_lists[3][0], angle_lists[3][1], angle_lists[3][2], angle_lists[3][3],
                                angle_lists[0][0], angle_lists[0][1], angle_lists[0][2], angle_lists[0][3]]  # Flatten the list if necessary
                joy_msg.buttons = []  # Optionally, you can also publish button states if relevant
                ...
                right_hand_ref_point = np.squeeze(np.asarray(right_joints.positions[1]))
