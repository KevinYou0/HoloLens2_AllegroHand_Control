...
if __name__ == '__main__':
    ...
    hand_pos_publisher = rospy.Publisher('/gripper_control_panda1', Joy, queue_size=10)
    ...
    try:
        ...
        while enable and not rospy.is_shutdown():
            ...
            if (si.is_valid_hand_right()):
                hand_detect = si.get_hand_right()
                hand_joints = hl2ss_utilities.si_unpack_hand(hand_detect)
                point_global= [hand_joints.positions[1][0],
                               hand_joints.positions[1][1],
                               hand_joints.positions[1][2]]
                thumb_positions = [hand_joints.positions[0], hand_joints.positions[1], hand_joints.positions[2],
                                   hand_joints.positions[3], hand_joints.positions[4], hand_joints.positions[5]]
                index_positions = [hand_joints.positions[6], hand_joints.positions[7], hand_joints.positions[8], hand_joints.positions[9], hand_joints.positions[10]]
                middle_positions = [hand_joints.positions[11], hand_joints.positions[12], hand_joints.positions[13], hand_joints.positions[14], hand_joints.positions[15]]
                ring_positions = [hand_joints.positions[16], hand_joints.positions[17], hand_joints.positions[18], hand_joints.positions[19], hand_joints.positions[20]]
                ...
                right_image_points = hl2ss_3dcv.project(hand_joints.positions, world_to_image)
                ...
                joy_msg = Joy()
                joy_msg.header.stamp = rospy.Time.now()
                index_rot = get_finger_rotation_angles([hand_joints.positions[17], hand_joints.positions[7], hand_joints.positions[10]])
                middle_rot = get_finger_rotation_angles([hand_joints.positions[7], hand_joints.positions[12], hand_joints.positions[15]])
                ring_rot = get_finger_rotation_angles([hand_joints.positions[7], hand_joints.positions[17], hand_joints.positions[20]])
                ...
                right_hand_ref_point = np.squeeze(np.asarray(hand_joints.positions[0]))
                ...
                points1 = np.squeeze(np.asarray(hand_joints.positions[0]))
                points2 = np.squeeze(np.asarray(hand_joints.positions[6]))
                points3 = np.squeeze(np.asarray(hand_joints.positions[21]))

                return_up_norm = calculate_plane_normal(points1, points2, points3)
                return_forward_norm = ((np.squeeze(np.asarray(hand_joints.positions[1])) - np.squeeze(np.asarray(hand_joints.positions[0]))) /
                                       norm(np.squeeze(np.asarray(hand_joints.positions[1])) - np.squeeze(np.asarray(hand_joints.positions[0]))))
