...
def get_joint_angles_thumb(point_positions):
    ...
    magnitude_BA = np.linalg.norm(BA)
    magnitude_BC = np.linalg.norm(BC)
    angle_radians = np.arccos(dot_product / (magnitude_BA * magnitude_BC)) * 1.2
    angle_list.append((3.14 - angle_radians))

    # joint 1
    angle_list.append(1.57)
    ...
    try:
        ...
        while enable and not rospy.is_shutdown():
            ...
            if (si.is_valid_hand_right()):
                ...
                hl2ss_utilities.draw_points(image, right_image_points.astype(np.int32)[[0, 1, 2,3,4,5]], radius,
